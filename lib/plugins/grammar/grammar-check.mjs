...
export class GrammarCheckPlugin extends BasePlugin {
   ...
   */
  generatePrompt(text) {
    return `请检查以下文本中的语言问题，包括语法错误、用词不当、表达不清、逻辑问题、标点符号错误等。
...

{
  ...
  "issues": [
    {
      "type": "grammar|punctuation|wording|logic",
      "line": 行号,
      "original": "原始文本片段",
      "revised": "修改后的文本片段",
      "description": "问题描述",
      ...
      return {
        sections: {
          ...
          analysis: issues.map(issue => 
            `${issue.line ? '行' + issue.line : '未知位置'}-${issue.description}`
          ).join('\n'),
  ...
  async process(input) {
    ...
    try {
      ...
      return this.formatOutput(
        ...
        [{
          type: 'error',
          description: `语法检查失败: ${error.message}`,
          severity: 'high'
        }],
        {
          ...metadata,
