...
export class PersonnelCheckPlugin extends BasePlugin {
  ...
  async fetchPersonnelData() {
    ...
    // 检查缓存是否有效
    const now = Date.now()
    if (this.personnelData && this.lastFetchTime && 
  ...
  normalizePersonnelData(rawData) {
    ...
    return dataArray.map(person => ({
      ...
      department: person.department || person.部门 || person.dept || '',
      id: person.id || person.工号 || person.employeeId || '',
      email: person.email || person.邮箱 || '',
      phone: person.phone || person.电话 || person.mobile || ''
  ...
  generatePrompt(text, personnelData) {
    const personnelList = personnelData.map(person => 
      `${person.name} - ${person.position}${person.department ? ` (${person.department})` : ''}`
    ).join('\n')

    return `请核对以下文本中提到的人员姓名和职务是否正确。
...

{
  ...
  "issues": [
    {
      "type": "name_error|position_error|not_found",
      "line": 行号,
      "original": "原始文本中的人员信息",
      "revised": "正确的人员信息",
      "description": "问题描述",
  ...
  "summary": "核对说明概要",
  "stats": {
    "totalMentioned": 提到的人员总数,
    "totalIssues": 问题总数,
    "correctPersonnel": 正确的人员数量,
    "issuesByType": {
      "name_error": 姓名错误数量,
      "position_error": 职务错误数量,
      "not_found": 未找到人员数量
    }
  }
}
...
- 如果没有问题，issues数组为空，但revisedText仍需包含完整原文
...
${text}`
      ...
      return {
        sections: {
          checkResult: issues.map(issue => 
            `${issue.type}-${issue.personName || '未知'}-${issue.description}`
          ).join('\n'),
          revisedText,
  ...
  async process(input) {
    ...
    try {
      ...
      // 调用AI模型
      const aiResponse = await this.modelManager.callModel(
        this.modelName,
        prompt,
        {
      ...
      return this.formatOutput(
        ...
        [{
          type: 'error',
          description: `人员核对失败: ${error.message}`,
          severity: 'high'
        }],
        {
          ...metadata,
  ...
  getCacheStatus() {
    return {
      ...
      isExpired: this.lastFetchTime ? 
        (Date.now() - this.lastFetchTime) > this.cacheExpiry : true
    }
  }
}