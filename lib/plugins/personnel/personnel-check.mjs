...
import { parseLocalPersonnel } from "../../utils/personnel-parser.mjs";
...
export class PersonnelCheckPlugin extends BasePlugin {
  // 静态元数据
  static pluginType = "personnel";
  static description = "核对文本中的人员姓名和职务信息是否正确";
  static version = "1.0.0";
  static supportedModels = ["gemini", "deepseek"];
  ...
  constructor(config = {}) {
    super({
      description: PersonnelCheckPlugin.description,
      version: PersonnelCheckPlugin.version,
      ...config,
    });
    ...
    // 本地名单配置
    this.useLocalPersonnel = config.useLocalPersonnel || false;
    this.localPersonnelData = config.localPersonnelData || "";
    this.parsedPersonnelData = config.parsedPersonnelData || null;
  ...
  async getPersonnelData() {
    // 如果启用了本地名单，优先使用前端解析好的数据
    if (this.useLocalPersonnel) {
      if (this.parsedPersonnelData && Array.isArray(this.parsedPersonnelData)) {
        console.log(`Using pre-parsed personnel data: ${this.parsedPersonnelData.length} records`);
        return this.parsedPersonnelData;
      }

      // 如果没有解析好的数据，回退到本地解析
      const localData = parseLocalPersonnel(this.localPersonnelData);
      console.log(`Using local personnel data (fallback): ${localData.length} records`);
      return localData;
    }

    // 否则从API获取
    return await this.fetchPersonnelData();
  }
...
<规则描述>
如果用户提供的文本里面包含参会人员名单, 你需要核对人员姓名和职务是否正确, 否则不进行核对。
</规则描述>
...
${text}
  ...
  async process(input) {
    ...
    try {
      // 获取人员数据（支持本地数据）
      const personnelData = await this.getPersonnelData();
      ...
      return this.formatOutput(sections.revisedText, changes, {
        ...
        dataSource: this.useLocalPersonnel ? "local" : this.personnelApiUrl,
        lastDataFetch: this.useLocalPersonnel ? "N/A" : new Date(this.lastFetchTime).toISOString(),
      });
    } catch (error) {
      ...
      // 对于本地名单，API URL 不是必需的
      if (error.message.includes("API URL not configured") && !this.useLocalPersonnel) {
        throw error;
      }
