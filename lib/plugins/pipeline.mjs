...
export class PluginPipeline {
  ...
  async execute(input) {
    ...
    const results = []
    const errors = []
    const summaries = [] // 存储所有插件的summary
    ...
    for (let i = 0; i < this.plugins.length; i++) {
      ...
      try {
        ...
        if (result.metadata && result.metadata.summary) {
          summaries.push(`[${plugin.name}] ${result.metadata.summary}`)
        }

        // 将当前插件的输出作为下一个插件的输入
        // summary需要累加，其他metadata正常合并
        const { summary: currentSummary, ...currentMetadata } = currentInput.metadata || {}
        const { summary: resultSummary, ...resultMetadata } = result.metadata || {}
        
        currentInput = {
          text: result.text, // revisedText：使用最新插件的输出
          context: { ...currentInput.context, ...result.context },
          metadata: { 
            ...currentMetadata, 
            ...resultMetadata,
            // summary累加：保留所有插件的处理记录
    ...
    return {
      ...
      finalText: currentInput.text, // 最终文本：最后一个插件的输出
      ...
      metadata: {
        ...
        ...currentInput.metadata // 包含最后状态的其他metadata
      }
    }
  }
